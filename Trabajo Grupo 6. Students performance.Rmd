...
```{r histogramas1, echo=FALSE}
# Crear los histogramas para cada categoría de nivel educativo de los padres
niveles_educativos <- unique(SP$parental_level_of_education)  

for (i in niveles_educativos) {
  datos_nivel <- SP %>% filter(parental_level_of_education == nivel)  
  
  # Error 3: No se ha cargado el paquete ggplot2
  histograma <- ggplot(datos_nivel, aes(x = as.numeric(Desempeño))) + 
    geom_histogram(aes(y = ..density..), binwidth = 10, fill = "lightblue", color = "black") +
    geom_density(color = "red", size = 1) +
    geom_line(stat = "density", color = "blue", linetype = "dashed", size = 1) +  
    labs(title = paste("Histograma de Desempeño para", nivel), 
         x = "Desempeño",
         y = "Densidad") +
    theme_minimal()
